...
@section('title', 'Edit Question - Eduria')
    ...
    <div class="row justify-content-center">
        <div class="col-lg-9">
            ...
            <div class="form-card shadow-sm">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('tentor.quizzes.questions.update', [$quiz->id, $question->id]) }}" id="questionForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control form-input @error('question_text') is-invalid @enderror"
                                      ...
                                      placeholder="Enter the question..." required>{{ old('question_text', $question->question_text) }}</textarea>
                            @error('question_text')
...
(function() {
    ...
    let options = [];
    let correctOptions = [];
    let currentType = '{{ old('type', $question->type) }}';
    ...
    function initFromQuestion() {
        const qOpts = @json(old('options', $question->options));
        const qCorrect = @json(old('correct_options', $question->correct_options));

        if (qOpts && Object.keys(qOpts).length > 0) {
            for (const [key, text] of Object.entries(qOpts)) {
                options.push({ key, text: text || '' });
            }
        }
        correctOptions = qCorrect && qCorrect.length > 0 ? [...qCorrect] : [];
    }
    ...
    function renderOptions() {
        ...
        if (!hasCorrect && options.length > 0 && !isMulti) {
